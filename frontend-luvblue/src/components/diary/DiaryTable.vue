<template>
  <div class="bg-white border border-gray-200 rounded-xl shadow-theme-sm dark:bg-gray-900 dark:border-gray-800">
    ...
    <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
      <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Diary Entries</h3>
      <button
        @click="$emit('create')"
    ...
    <div v-if="loading" class="p-8 text-center">
      ...
      <p class="text-gray-500 dark:text-gray-400">Loading diary entries...</p>
    </div>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          <tr v-if="items.length === 0">
            <td colspan="5" class="px-5 py-8 text-center text-gray-500 dark:text-gray-400">
              No diary entries found. Click 'Create Entry' to add one.
            </td>
          </tr>
          <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
            <td class="px-5 py-4">
              <div class="w-16 h-16 rounded overflow-hidden bg-gray-100 flex-shrink-0 border border-gray-200 dark:border-gray-700">
                <img v-if="item.diary_image_url" :src="getImageUrl(item.diary_image_url)" class="w-full h-full object-cover" />
            ...
            <td class="px-5 py-4">
              <div class="font-medium text-gray-900 dark:text-white max-w-[200px] truncate" :title="item.diary_title">
                {{ item.diary_title }}
              </div>
            </td>
            <td class="px-5 py-4">
              <div class="text-gray-500 dark:text-gray-400 max-w-[150px] truncate" :title="item.diary_brief_title">
                {{ item.diary_brief_title || '-' }}
              </div>
            </td>
            <td class="px-5 py-4">
              <div class="flex items-center truncate">
                <a
                  v-if="item.diary_instagram_link"
                  :href="item.diary_instagram_link"
                  target="_blank"
